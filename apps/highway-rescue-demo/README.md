...
## 技术选型
- **Vue 3（ESM 模块化）**：主界面模板与业务逻辑解耦，采用 `app.js` + `app-data.js` + `map-engine.js` 分层组织
...
- `docs/frontend-framework-requirements.md`（全仓统一前端架构要求）

## 前端架构（本次补强）
- `static/demo/index.html`：仅保留视图结构与样式
- `static/demo/app.js`：Vue 应用状态、交互编排、时间轴与搜索逻辑
- `static/demo/app-data.js`：巡检时序数据与实体样本
- `static/demo/map-engine.js`：Leaflet 地图渲染引擎（图层、热力、链路、实体标注）
