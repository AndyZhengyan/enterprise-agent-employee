...
class TestMemoryManager:
    ...
    def test_session_limit(self):
        ...
        # 创建两个会话
        manager.get_or_create("sess-1", "agent-001", "user-001")
        manager.get_or_create("sess-2", "agent-001", "user-001")

        assert len(manager.sessions) == 2

        # 创建第三个会话时，最旧的应该被清理
        manager.get_or_create("sess-3", "agent-001", "user-001")
    ...
    def test_delete_session(self):
        """删除会话"""
        manager = MemoryManager()
        manager.get_or_create("sess-xxx", "agent-001", "user-001")

        assert len(manager.sessions) == 1

        deleted = manager.delete("sess-xxx")
...
class TestMemoryCompression:
    ...
    def test_compression_trigger(self):
        """压缩触发条件"""
        manager = MemoryManager()
        ctx = manager.get_or_create("sess-xxx", "agent-001", "user-001")
    ...
    def test_auto_compression_on_access(self):
        """访问时自动压缩"""
